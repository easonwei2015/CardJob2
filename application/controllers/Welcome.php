...
class Welcome extends CI_Controller {
	...
	var $index_base_url = '';
	var $host_name = '玉里慈濟醫院工作卡上傳系統'; // 系統名稱
	var $manage_group = array('Lin'); // 管理者權限
	var $upload_dir ="assets/upload_data/card_upload/"; // 工作卡上傳路徑
	var $review_dir ="assets/upload_data/review_card/"; // 已審核工作卡上傳路徑
	var $upload_del_dir ="assets/del/card_upload/"; // 工作卡刪除路徑
	var $review_del_dir ="assets/del/review_card/"; // 已審核工作卡刪除路徑
	...
	public function index()
	{
		...
		$data['page_name'] = '公告';		
		$data['files_ranking'] = $this->main_model->files_ranking(); // 上傳排行榜
		//$data['review_ranking'] = $this->main_model->review_ranking(); // 審核排行榜
	...
	public function card_upload()
	{
		...
		$data['download_link'] = $this->upload_dir;		
		//$data['errors'] = $errors;
	...
	public function review_card()
	{
		...
		$data['show_next_page']= ($PageTotal>$next_page)? "<li><a href='".$this->index_base_url."card_upload?page=".$next_page."&datepicker=".$data['datepicker']."&users=".$data['users']."'>下一頁</a></li>" : "";
		$data['manage_group'] = (in_array(trim($session['ssnUserName']), $this->manage_group)) ? true : false;		
		$data['download_link'] = $this->review_dir;
			...
			foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
				...
				if(empty($errors)==true){
					if(is_dir($this->upload_dir)==false){
						...
						mkdir($this->upload_dir, 0, true);
					}
					//add files
					if(is_file("$this->upload_dir".$file_name)==false){
					
						move_uploaded_file($file_tmp,$this->upload_dir.$file_name);
					...
					// modify files
						...
						// 有人上傳過此檔名，自己還未上傳過
							...
							move_uploaded_file($file_tmp,$this->upload_dir.$ver_name);	//add files					
							$insert_data = array(
											'file_name' =>$ver_name,
											'author' =>$session['ssnUserName'],
											'table' => $post_data['table'],
						...
						$new_dir= $this->upload_dir.$file_name;
	...
	public function review_do_upload(){
		...
		if(isset($_FILES['files'])){
			...
			foreach($_FILES['files']['tmp_name'] as $key => $tmp_name ){
				...
				if(empty($errors)==true){
					if(is_dir($this->review_dir)==false){
						//mkdir("$desired_dir", 0777);
						mkdir($this->review_dir, 0, true);
					}
					//add files
					if(is_file("$this->review_dir".$file_name)==false){
					
						move_uploaded_file($file_tmp,$this->review_dir.$file_name);
					...
					// modify files
						...
						$new_dir= $this->review_dir.$file_name;
						 rename($file_tmp,$new_dir);
	...
	{		
		...
		if($get_data['page'] == 'card_upload'){
			$del_dir = $this->upload_del_dir;
			$desired_dir= $this->upload_dir;
		}elseif($get_data['page'] == 'review_card'){
			$del_dir = $this->review_del_dir;
			$desired_dir= $this->review_dir;
		}	
	...
	private function _check_browser(){
		if(!strpos($_SERVER['HTTP_USER_AGENT'],"Chrome")){
			echo '<H1><span style="color:red">請使用 Google Chrome 瀏覽器<img border="0" src="'.base_url().'assets/img/chrome.png" width="25">，以免功能無法正常使用。</span><H1>';
		}		
	}
}
